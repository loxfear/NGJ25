...
namespace Coherence.Runtime
{
    ...
    public class RequestException : Exception
    {
        ...
        public RequestException(ErrorCode errorCode, HttpStatusCode statusCode = default, string userMessage = null, bool retry = false)
        {
            ErrorCode = errorCode;
            HttpStatusCode = statusCode;
        ...
        public RequestException(int statusCode, string message) : this((HttpStatusCode)statusCode, message) { }
        public RequestException(ErrorCode errorCode, int statusCode, string userMessage = null, bool retry = false) : this(errorCode, (HttpStatusCode)statusCode, userMessage, retry) { }
        ...
        public override string Message
        {
            get
            {
                ...
                return ErrorCode switch
                {
                    ...
                    ErrorCode.LoginWeakPassword => "Password given is invalid. Password cannot be empty.",
#if UNITY
                    ErrorCode.RestrictedModeCapReached => $"Total user capacity for restricted mode server reached. See the documentation for more information: {DocumentationLinks.GetDocsUrl(DocumentationKeys.UnlockToken)}",
#else
        ...
        public static bool TryParse(string response, int statusCode, out RequestException requestException, Logger logger)
        {
            ...
            if (errorResponse.ErrorCode == ErrorCode.Unknown)
            {
                ...
                if (string.IsNullOrEmpty(errorResponse.UserMessage))
                {
                    requestException = null;
                    return false;
                }
            ...
            requestException = new RequestException(errorResponse.ErrorCode, statusCode, errorResponse.UserMessage, errorResponse.Retry);
            return true;
        }
    }
