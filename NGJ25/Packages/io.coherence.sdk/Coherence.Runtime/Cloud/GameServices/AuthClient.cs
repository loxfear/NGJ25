...
namespace Coherence.Cloud
{
    ...
    public sealed class AuthClient : IAuthClientInternal, IDisposableInternal
    {
        ...
        private static readonly UserGuid SimulatorInCloudUniqueId = new(SimulatorInCloudUniqueIdValue);
        private const string SimulatorInCloudUniqueIdValue = "SimulatorInCloud";
        ...
        private User user;
        private readonly UserGuid uniqueId;
        ...
        {
            var authClient = new AuthClient(requestFactory, projectId: projectId, uniqueId: new(uniqueId));
        ...
        public AuthClient(string uniqueId, bool autoLoginAsGuest, string projectId, IRequestFactory requestFactory) : this(requestFactory, projectId:projectId, uniqueId:new(uniqueId))
        {
            if (!autoLoginAsGuest)
            {
                return;
        ...
        private AuthClient(IRequestFactory requestFactory, string projectId = "", UserGuid uniqueId = default, SessionToken sessionToken = default)
        {
            this.OnInitialized();
            this.requestFactory = requestFactory;
            this.user = new User(uniqueId, "", "", sessionToken);
            this.uniqueId = uniqueId ?? UserGuid.None;
        ...
        public void Dispose()
        {
            ...
            try
            {
                if (!Prefs.Save())
                {
                    logger.Warning(Warning.PrefsFailedToSave, ("reason", "No data to save."));
                }
            }
            catch (Exception ex)
            {
...
#pragma warning restore CS8524
        ...
        private LoginResult HandleLogin(string username, string guestPassword, string response)
        {
            ...
            var success = new LoginResult(uniqueId, username, guestPassword, Result.Success, loginResponse);
            user = success.User;
