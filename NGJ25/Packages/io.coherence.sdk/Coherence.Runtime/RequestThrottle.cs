    ...
    internal class RequestThrottle
    {
        ...
        public virtual async Task WaitForCooldown(string basePath, string method, CancellationToken cancellationToken)
        {
            while(true)
            {
                ...
                await UnityEngine.Awaitable.WaitForSecondsAsync((float)cooldown.TotalSeconds, cancellationToken);
                #else
                // Avoid Task.Delay because it might cause crashing in WebGL builds.
                await Task.Yield();
                #endif
            }
