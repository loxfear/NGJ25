...
namespace Coherence.Toolkit.Tests
{
    ...
    public class CoherenceSyncTests : CoherenceTest
    {
        ...
        [Test]
        [TestCaseSource(nameof(commonTestCases))]
        public void ReceiveCommand_Reflected_HandlesRouting(MessageTarget commandTarget, MessageTarget routing, bool expectReceived)
        {
            // Arrange
            ...
            sync.usingReflection = true;
            sync.CallAwake();
            sync.EntityState = new NetworkEntityState(Entity.InvalidRelative, AuthorityType.Full, false, false, sync, String.Empty);

            var descriptor = new Descriptor(sync.GetType(), new DynamicMethod(commandName, typeof(int), Array.Empty<Type>()));

            var binding = descriptor.InstantiateBinding(sync);
            binding.routing = routing;

            sync.Bindings.Add(binding);

            int commandsReceived = 0;
            sync.NetworkCommandReceived += (sender, data) => commandsReceived++;

            // Act
            try
            {
                sync.ProcessGenericNetworkCommand(binding.guid, commandTarget, new byte[] { }, new Entity[] { });
            }
            catch (Exception) when (commandsReceived > 0)
            {
            }

            // Assert
            Assert.That(commandsReceived, Is.EqualTo(expectReceived ? 1 : 0));
        }
