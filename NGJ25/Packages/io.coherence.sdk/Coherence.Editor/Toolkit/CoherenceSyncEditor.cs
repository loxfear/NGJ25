    ...
    internal partial class CoherenceSyncEditor : BaseEditor
    {
        ...
        private static partial class GUIContents
        {
            ...
            public static readonly GUIContent bakeScriptReady =
                EditorGUIUtility.TrTextContentWithIcon("Using optimized generated code.", "Installed");

            public static readonly GUIContent bakedScriptNotFound =
                EditorGUIUtility.TrTextContentWithIcon("Optimized code not found. Bake to fix.", "Warning");
        ...
        protected class UnityEventInfo
        {
            ...
            internal void Draw(Object target)
            {
                using (var relativeProp = prop.FindPropertyRelative("m_PersistentCalls.m_Calls"))
                {
                    ...
                    {
                        EditorGUILayout.LabelField(content, ContentUtils.GUIStyles.centeredLabel);
        ...
        private SerializedProperty usingReflectionProperty;
        ...
        protected override void OnEnable()
        {
            ...
            usingReflectionProperty = serializedObject.FindProperty("usingReflection");
        ...
        protected override void OnDisable()
        {
            ...
            usingReflectionProperty.Dispose();
        ...
        {
            _ = EditorGUILayout.BeginVertical(GUI.skin.box);
            EditorGUILayout.LabelField(GUIContents.notMappedPrefabMode, ContentUtils.GUIStyles.centeredLabel,
        ...
        private void DrawBakeWarning()
        {
            ...
            var needsReflectionModeBaking = sync.usingReflection && !BakeUtil.HasReflectedModeSupport;
            var needsBakedModeBaking = !sync.usingReflection && ProjectSettings.instance.ActiveSchemasChanged;
            if (needsReflectionModeBaking || needsBakedModeBaking)
            {
                _ = EditorGUILayout.BeginVertical(GUI.skin.box);
        ...
        private void DrawBindingActions()
        {
            ...
            {
                using (new EditorGUI.DisabledScope(sync.usingReflection))
                {
                    if (GUILayout.Button(!sync.usingReflection
                            ? GUIContents.optimize
                            : new GUIContent(GUIContents.optimize)
                            {
                                tooltip = "Requires Baked Scripts",
                            }, GUIStyles.button))
                    {
                        BindingsWindow.Init();
        ...
        private void DrawBakedScript()
        {
            ...
            var rect = EditorGUILayout.GetControlRect(true, EditorGUIUtility.singleLineHeight, EditorStyles.toggle);
            var label = EditorGUI.BeginProperty(rect, GUIContents.bakedScript, usingReflectionProperty);
            EditorGUI.BeginChangeCheck();
            var sel = EditorGUI.Toggle(rect, label, !usingReflectionProperty.boolValue);
            if (EditorGUI.EndChangeCheck())
            {
                ReportAnalytic("baked_mode", sel);
                usingReflectionProperty.boolValue = !sel;
            }
            ...
            if (!usingReflectionProperty.boolValue)
            {
                EditorGUI.indentLevel++;
