    ...
    internal class CoherenceBridgeEditor : BaseEditor
    {
        ...
        private void DrawClientConnectionSettings()
        {
            ...
            static void HandleChangeClientConnectionAuthorityType(SerializedProperty serializedProperty, string userWasPromptedToUpdateSessionStateKey)
            {
                ...
                const string errorMessage = "Connection prefabs must have 'Authority Type' set to '"+ nameof(CoherenceSync.AuthorityTransferType.NotTransferable) +"'.";
                EditorGUILayout.HelpBox(errorMessage, MessageType.Warning);

                if (SessionState.GetBool(userWasPromptedToUpdateSessionStateKey, false))
                {
                    return;
                }

                SessionState.SetBool(userWasPromptedToUpdateSessionStateKey, true);

                // Delay DisplayDialog to avoid GUI layout warnings
                EditorApplication.delayCall += () =>
                {
                    if (EditorUtility.DisplayDialog("Unsupported Authority Type",
                            $"The connection prefab \"{clientConnectionPrefabSync.name}\" has 'Authority Type' set to '{clientConnectionPrefabSync.authorityTransferType}'. Connection prefabs must have 'Authority Type' set to '{nameof(CoherenceSync.AuthorityTransferType.NotTransferable)}'.\n\n" +
                            "Would you like 'Authority Type' to be changed to 'Not Transferable' for the prefab?", "Yes, change", "No"))
                    {
                        ...
                        clientConnectionPrefabSyncSerializedObject.ApplyModifiedProperties();
                    }
                };
            }
        }
