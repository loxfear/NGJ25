...
namespace Coherence.Editor.Toolkit
{
    ...
    internal class CoherenceSyncBindingsWindow : InspectComponentWindow<CoherenceSync>
    {
        ...
        private Dictionary<string, List<Binding>> bindingsWithMissingComponents =
            new Dictionary<string, List<Binding>>();
        ...
        protected override void OnGUI()
        {
            ...
            if (Application.isPlaying && !Sync.usingReflection)
            {
        ...
        public void DrawMainView()
        {
            if (scope == Scope.Components)
            {
                if (componentActionsWindow)
                {
                    ...
                    if (GUI.enabled)
                    {
                        GUILayout.Label(GetScopeInfoContent(), ContentUtils.GUIStyles.centeredLabel);
                    }
        ...
        public void DrawBindings()
        {
            ...
            for (var i = 0; i < componentsProperty.arraySize; i++)
            {
                ...
                var canEdit = (!Application.isPlaying || Sync.usingReflection) &&
                              (!(isInstance && !isAsset) || Application.isPlaying);
            ...
            var enableBindingsWithoutComponents = (!Application.isPlaying || Sync.usingReflection) &&
                          (!(isInstance && !isAsset) || Application.isPlaying);
            EditorGUI.BeginDisabledGroup(!enableBindingsWithoutComponents ||
                                         (component && TypeUtils.IsNonBindableType(component.GetType())));
            DrawBindingsWithoutComponents();
            EditorGUI.EndDisabledGroup();
        }
        ...
        private void DrawComponentBindings(Component component)
        {
            ...
            for (var i = 0; i < descriptors.Count; i++)
            {
                ...
                if (CanEdit)
                {
                    ...
                    if (Event.current.alt)
                    {
                        ...
                        rect.y += 2;
                        rect.width = 16;
                        GUI.Label(rect, ContentUtils.GUIContents.binding);
                    }
            ...
            for (var i = 0; i < Sync.Bindings.Count; i++)
            {
                ...
                var controlLabel = EditorGUI.BeginProperty(controlRect,
                    ContentUtils.GetInvalidContent(binding.Descriptor, invalidReason), serializedProperty);

                if (CanEdit)
                {
                    EditorGUI.BeginChangeCheck();
                    bool selected = EditorGUI.ToggleLeft(controlRect, controlLabel, true,
                        ContentUtils.GUIStyles.richLabelNoWrap);
                    if (EditorGUI.EndChangeCheck())
                    {
                        if (!selected)
        ...
        private void DrawInvalidBindings()
        {
        ...
        private void DrawBindingsWithoutComponents()
        {
            bindingsWithMissingComponents.Clear();
            ...
            foreach (var binding in Sync.Bindings)
            {
                ...
                if (binding.Descriptor.OwnerType == null || binding.UnityComponent == null)
                {
                    if (!bindingsWithMissingComponents.ContainsKey(binding.Descriptor.OwnerAssemblyQualifiedName))
                    {
                        bindingsWithMissingComponents.Add(binding.Descriptor.OwnerAssemblyQualifiedName,
                            new List<Binding>());
                    }

                    bindingsWithMissingComponents[binding.Descriptor.OwnerAssemblyQualifiedName].Add(binding);
                }
            }

            foreach (var kv in bindingsWithMissingComponents)
            {
                _ = EditorGUILayout.BeginHorizontal(GUI.skin.box);
                var content =
                    EditorGUIUtility.TrTextContentWithIcon($"Missing script ({kv.Key.Remove(kv.Key.IndexOf(','))})",
                        "Warning");
                EditorGUILayout.LabelField(content, EditorStyles.boldLabel);
                EditorGUILayout.EndHorizontal();

                DrawMissingComponentBindings(Sync, kv.Value);
            }
        }

        private void DrawMissingComponentBindings(CoherenceSync sync, List<Binding> bindingsWithoutComponent)
        {
            EditorGUI.indentLevel++;
            for (var i = 0; i < bindingsWithoutComponent.Count; i++)
            {
                var binding = bindingsWithoutComponent[i];
                ...
                var baseReason = "Script associated with this binding is missing.";
                var recoverable = "(Recoverable)";
                var notRecoverable = "(Not Recoverable)";
                var suffixReason = binding.UnityComponent != null ? recoverable : notRecoverable;

                var content = ContentUtils.GetInvalidContent(binding.Descriptor, $"{baseReason} {suffixReason}");
                if (CanEdit)
                {
                    EditorGUI.BeginChangeCheck();
