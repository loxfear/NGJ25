...
namespace Coherence.Build
{
    ...
    public static class SimulatorBuildPipeline
    {
        ...
        public static void PrepareHeadlessBuild()
        {
            ...
            ChangeBuildSubTarget(supportsHeadlessLinuxBuild, false);
            ChangeBuildTargetToLinux();
        }
        ...
        public static void BuildHeadlessLinuxClientAsync()
        {
            ...
            var changedBuildSubTarget = ChangeBuildSubTarget(supportsHeadlessLinuxBuild, false);
        ...
        public static void BuildLocalSimulator(bool forceShowPrompt = false)
        {
            ...
            bool changedBuildSubTarget = ChangeBuildSubTarget(options.HeadlessMode, true);
        ...
        private static bool ChangeBuildSubTarget(bool useHeadlessMode, bool useReflection)
        {
            if (EditorUserBuildSettings.standaloneBuildSubtarget != StandaloneBuildSubtarget.Server && useHeadlessMode)
            {
                if (useReflection)
                {
                    ChangeSubTargetViaReflection(StandaloneBuildSubtarget.Server);
                }
            ...
            if (EditorUserBuildSettings.standaloneBuildSubtarget != StandaloneBuildSubtarget.Player && !useHeadlessMode)
            {
                if (useReflection)
                {
                    ChangeSubTargetViaReflection(StandaloneBuildSubtarget.Player);
                }
        ...
        private static void ChangeSubTargetViaReflection(StandaloneBuildSubtarget subtarget)
        {
            // WORKAROUND FOR UNITY 2021 BUG
            // --------
            // Unity uses two internal methods to check the selected sub target:
            //  - EditorUserBuildSettings.GetSelectedSubtargetFor(BuildTarget target)
            //  - EditorUserBuildSettings.GetActiveSubtargetFor(BuildTarget target)
            // When you change the subtarget programatically via EditorUserBuildSettings.standaloneBuildTarget, the first method
            // will return the incorrect value, and the second method will return the correct value. If you use the BuildPipeline API,
            // Unity will check the subtarget using GetActiveSubtargetFor, which returns the correct value. But if you use the DefaultBuildMethod,
            // Unity will check the subtarget using GetSelectedSubtargetFor, which returns the incorrect value, so we need to use the internal setter
            // to bypass the bug.
            var method = typeof(EditorUserBuildSettings).GetMethod("SetSelectedSubtargetFor",
                BindingFlags.Static | BindingFlags.NonPublic);

            var objects = new object[2];
            objects[0] = EditorUserBuildSettings.activeBuildTarget;
            objects[1] = (int)subtarget;
        ...
        private static void PerformHeadlessBuild()
        {
            ...
            {
                EditorUtility.DisplayDialog("Simulator Build Failed",
