...
namespace Coherence.Toolkit
{
    ...
    [NonBindable]
    [HelpURL("https://docs.coherence.io/v/1.5/manual/components/coherence-sync")]
    public sealed partial class CoherenceSync : CoherenceBehaviour, ICoherenceSync, IConnectedEntityDriver
    {
        ...
        bool ICoherenceSync.UseReflection => usingReflection;
...
#pragma warning restore CS0618
        ...
        /// </summary>
        /// <remarks>
        /// It is strongly encouraged to use baked mode.
        /// </remarks>
...
#pragma warning restore CS0618
        ...
        {
            if (bakedScript != null || usingReflection)
            {
            ...
            if (string.IsNullOrEmpty(bakedScriptType))
            {
                ...
                logger.Warning(Warning.ToolkitSyncBakedScriptNoTypeStored,
                    $"Couldn't load baked script for '{name}' (no type stored). Reimport the prefab to store baked script type information. Falling back to reflection.",
                    ("context", this));
                return false;
            }
            ...
            if (t == null)
            {
                ...
                $"Couldn't load baked script for '{name}'.\n" +
                    $"Check the {nameof(CoherenceSync)} Inspector of this Scene Object for possible troubleshooting.\n" +
            ...
            catch (Exception e)
            {
                logger.Error(Error.ToolkitSyncInstantiationException,
                    $"Encountered issues while instantiating the Baked Script of {name}. " +
                        $"If you have made changes to the networked data of the Prefab, you have to Bake from the menu item coherence/Bake.\n\n" +
        ...
        /// </summary>
        /// <remarks>
        /// An entity with defined LODs still won't benefit from them if <see cref="usingReflection"/> is set.
        /// </remarks>
        public bool UsesLODsAtRuntime => GeneratesArchetypeDefinition && !usingReflection;
        ...
        private void DestroyNetworkEntity()
        {
            ...
            // This makes sure that we'll remap reflected bindings
            updater = null;
        ...
        public void ProcessGenericNetworkCommand(string commandName, MessageTarget target, byte[] data, Entity[] entityIDs)
        {
            commandsHandler.HandleGenericCommand(commandName, target, data, entityIDs);
        }
        ...
        bool ICoherenceSync.ShiftOrigin(Vector3d delta)
        {
            ...
            {
                ((ICoherenceSync)this).Updater.TryFlushPosition(false); // Flushes new relative position to the change buffer
            }
