...
namespace Coherence.Toolkit
{
    ...
    public class EntitiesManager : IEntitiesManager
    {
        ...
        {
            if (!bridge.EnableClientConnections || bridge.GlobalQueryGameObject != null)
            {
                return;
            }

            bridge.GlobalQueryGameObject = new GameObject("[coherence] GlobalQuery");
            bridge.GlobalQueryGameObject.transform.SetParent(bridge.Transform);
            bridge.GlobalQueryGameObject.SetActive(false);

            CoherenceBridgeStore.instantiatingBridge = bridge;
            try
            {
                CoherenceSync sync = bridge.GlobalQueryGameObject.AddComponent<CoherenceSync>();
                sync.CoherenceSyncConfig = ScriptableObject.CreateInstance<CoherenceSyncConfig>();

                var id = "GlobalQuery";

                sync.CoherenceSyncConfig.Init(id);

                sync.CoherenceSyncConfig.Instantiator = new DefaultInstantiator();

                CoherenceSyncConfigRegistry.Instance.Register(sync.CoherenceSyncConfig);

                ICoherenceComponentData globalQuery = definition.GenerateGlobalQueryComponent();
                Entity entity = client.CreateEntity(new[]
                {
                    globalQuery
                }, false);
                var componentUpdates = ComponentUpdates.New(new Dictionary<uint, ComponentChange>
                {
                    [globalQuery.GetComponentType()] = ComponentChange.New(globalQuery)
                });

                var orphanedEntity = new UnsyncedNetworkEntity()
                {
                    EntityState = new NetworkEntityState(entity, AuthorityType.State, false, true, null, string.Empty),
                    Updates = componentUpdates,
                };

                if (!unsyncedNetworkEntities.TryGetValue(id, out var queue))
                {
                    queue = new Queue<UnsyncedNetworkEntity>();
                    unsyncedNetworkEntities.Add(id, queue);
                }

                queue.Enqueue(orphanedEntity);

                bridge.GlobalQueryGameObject.SetActive(true);
            }
            catch (Exception e)
            {
                throw new Exception($"Failed to initialize global query.\n{e}");
            }
            finally
            {
        ...
        private NetworkEntityState CreateEntity(ICoherenceSync sync, string uuid)
        {
            ...
            {
                var bakedScriptCreates = sync.BakedScript.CreateEntity(sync.UsesLODsAtRuntime, sync.ArchetypeName, bridge.NetworkTime.ClientSimulationFrame);
                if (bakedScriptCreates != null)
                {
                    components.AddRange(bakedScriptCreates);
                }
            }
