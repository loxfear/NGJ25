...
namespace Coherence.Toolkit
{
    ...
    [NonBindable]
    [HelpURL("https://docs.coherence.io/v/1.5/manual/authority/server-authoritative-setup")]
    public class CoherenceInput : MonoBehaviour, ICoherenceInput
    {
        ...
        private bool VerifyCoherenceSync()
        {
            ...
            {
                logger.Error(Error.ToolkitInputNoReflection,
                    $"Input requires Baked mode. Please make sure to enable it in the {nameof(CoherenceSync)} settings. Disabling input component.",
                    ("context", this));
                enabled = false;
                return false;
            }
