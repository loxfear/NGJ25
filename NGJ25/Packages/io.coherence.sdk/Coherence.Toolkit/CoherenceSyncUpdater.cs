    ...
    internal class CoherenceSyncUpdater : ICoherenceSyncUpdater
    {
        ...
        private void UpdateValueBindings()
        {
            ...
            foreach (var group in valueBindings.GroupBy(b => b.CoherenceComponentName))
            {
                ...
                foreach (var binding in group)
                {
                    if (coherenceSync.UseReflection)
                    {
                        binding.UpdateGenericPool(count++);

                        var assemblyType = binding.MonoAssemblyRuntimeType;
                        var size = TypeUtils.GetReflectedTypePoolSize(assemblyType);

                        if (count > size)
                        {
                            logger.Warning(Warning.ToolkitSyncUpdateUnsupportedReflection,
                                $"We can only synchronize {size} variables of type {assemblyType?.Name} " +
                                    $"on the same game object when using reflection. This binding will not be synced: {binding.Name}. " +
                                    $"Try switching to 'Use baked scripts' in the inspector.");
                            continue;
                        }
                    }
