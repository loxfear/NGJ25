...
namespace Coherence.Toolkit
{
    ...
    public class CommandsHandler
    {
        ...
        private bool UsingReflection => sync.UseReflection;
        ...
        public void HandleCommand(IEntityCommand command, MessageTarget target)
        {
            if (UsingReflection)
            {
                // Routing for reflection mode is validated in the ProcessGenericNetworkCommand
                // since IEntityCommand.Routing is not set and at this point we have not enough
                // info to find the command in the bindings.
                Impl.ReceiveGenericCommand(sync, command, target, logger);
            }
            else
            {
        ...
        public bool SendCommand(Type targetType, string methodName, MessageTarget target, ChannelID channelID, bool sendToAllBindings, params object[] args)
        {
            ...
            if (UsingReflection)
            {
                return SendCommandUsingReflection(commandName, target, channelID, options, args);
            }
            ...
            object receiver, string methodName)
        {
            ...
            SendOptions options = GetSendOptions(receiver as Component, sendToAllBindings);
        ...
        private bool GetCommandNameAndValidateEntityId(Type targetType, string methodName, out string commandName)
        {
            ...
            while (currentType != null && !foundValidCommandName)
            {
                ...
                if (registeredCommandNames.Contains(commandName) || UsingReflection)
                {
            ...
            if (!foundValidCommandName)
            {
                ...
                string GetErrorMessage(string commandName)
                {
                    if (ReflectionUtils.TryGetMethod(targetType, methodName, BindingFlags.Public | BindingFlags.Instance, out var publicMethod))
                    {
                        ...
                        {
                            return $"Unable to send command '{commandName}' to '{Name}': command was not found. Try rebuilding Network code using 'coherence > Bake'.";
                        }

                        return $"Unable to send command '{commandName}' to '{Name}': command was not found. Make sure that the method '{methodName}' has been marked as a command on the component of type '{targetType.FullName}' using the Configuration window, or add the [Command] attribute to it. After this, try rebuilding Network code using 'coherence > Bake'.";
                    }
        ...
        // Pure tuples version optimization skipping all reflection.
        private bool SendCommandUsingReflection(string commandName, MessageTarget target, ChannelID channelID, SendOptions options, params ValueTuple<Type, object>[] args)
        {
            // Validate
            bool canSendCommand = ValidateReflectedCommandSending(commandName, target, options, out List<CommandBinding> commandBindings);
            if (!canSendCommand)
            {
                return false;
            }
            ...
            return SendCommandUsingReflection(options.Receiver, commandName, target, channelID, finalArgs, finalArgTypes, commandBindings);
        }

        // Mixed object version where args could be tuples using reflection.
        private bool SendCommandUsingReflection(string commandName, MessageTarget target, ChannelID channelID, SendOptions options, params object[] args)
        {
            // Validate
            bool canSendCommand = ValidateReflectedCommandSending(commandName, target, options, out List<CommandBinding> commandBindings);
            if (!canSendCommand)
        ...
        private bool SendCommandUsingReflection(Component specifiedReceiver, string commandName, MessageTarget target, ChannelID channelID, object[] args, Type[] types, List<CommandBinding> commandBindings)
        {
            ...
            {
                if (specifiedReceiver != null && commandBinding.UnityComponent != specifiedReceiver)
                {
                    continue;
                }

                MethodInfo method = commandBinding.GetMethodInfo();
                Component receiver = commandBinding.UnityComponent;

                var expectedArgs = method.GetParameters();
                if (expectedArgs.Length != args.Length)
                {
                    logger.Error(Error.ToolkitSyncCommandArgCountMismatch,
                        $"command arguments count mismatch for {commandName}. Expected {expectedArgs.Length} but got {args.Length}");
                ...
                {
                    if (expectedArgs[i].ParameterType != types[i])
                    {
                        logger.Error(Error.ToolkitSyncCommandArgTypeMismatch,
                            $"command argument {i} type mismatch for {commandName}. Expected {expectedArgs[i].ParameterType} but got {types[i]}");
                ...
                {
                    method.Invoke(receiver, UnifyNullTypes(args, types));
                }

                if (sendTo.HasFlag(SendTo.Others))
                {
                    var (data, entityIDs) = GenericNetworkCommandArgs.SerializeCommandArgs(method, (CoherenceBridge)CoherenceBridge, args, logger);
                    if (data == null)
                    {
                        return false;
                    }

                    Impl.SendGenericCommand(sync, sync.CoherenceBridge.Client, commandBinding.guid, target, channelID, data, entityIDs, logger);
                }
            }
        ...
        private bool ValidateReflectedCommandSending(string commandName, MessageTarget messageTarget, SendOptions options, out List<CommandBinding> bindings)
        {
            bindings = GetCommandBindingsMemoized(commandName);

            if (bindings == null || bindings.Count == 0)
            {
                logger.Error(Error.ToolkitCommandSendReflectedBindings,
                    $"Trying to send command '{commandName}' to '{Name}', but it's not found. This message will not be sent.");
                return false;
            }

            if (options.Receiver == null && !ValidateNumberOfBindings(commandName, options.SendToAllBindings, bindings))
            {
                return false;
            }
            ...
            {
                if (!CanRouteMessage(messageTarget, binding.routing))
                {
            ...
            if (!ValidateOrphan(commandName, messageTarget))
            {
                return false;
            }
        ...
        private bool ValidateNumberOfBindings(string commandName, bool sendToAllBindings, List<CommandBinding> bindings)
        {
            if (!sendToAllBindings && bindings.Count > 1)
            {
                var errorBuilder = new StringBuilder();
                errorBuilder.AppendLine(
                    $"Trying to send a single command, but {commandName} is bound to more than one component. " +
                    "If it is intended, please use the SendCommandToComponentsInChildren method instead. Bound components: ");

                foreach (var binding in bindings)
                {
                    errorBuilder.AppendLine($"- {binding.UnityComponent.gameObject.name}");
                }

                logger.Error(Error.ToolkitCommandValidateBindings, errorBuilder.ToString());
                return false;
            }
